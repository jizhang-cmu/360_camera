...
Copy the 'start_360_camera.desktop' file in the 'desktop_button' folder to the desktop. Edit the 2 links to the '360_camera_sensorpod.sh' script and 'start.png' icon in the desktop file. Note that if using an add-on computer connected to the built-in NUC i7 computer via an Ethernet cable and the 360 camera plugged into the add-on computer, point the link to the '360_camera_ai_computer.sh' script instead. The script uses 'ROS_DOMAIN_ID=1' on the add-on computer and remaps the IMU topic to not use it in the 360 camera driver. Right-click the 'start_360_camera.desktop' file on the desktop and 'Allow Launching'. Double-click to launch the 360 camera driver.
